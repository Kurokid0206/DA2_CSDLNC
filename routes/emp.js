  ...
  ;(async () => {
    try {
      ...
      let result = await pool
        .request()
        .query(
          `Select * from HoaDon where NVGiaoHang is null and TrangThai = N'Chưa Giao'`,
        )
...
router.post('/get-received-order', function (req, res) {
  ;(async () => {
    try {
      let pool = await sql.connect(config)
      let result = await pool.request().query(`Select * from HoaDon where 
                NVGiaoHang = '${req.session.user}' and TrangThai = N'Ðang Giao'`) //thay bằng session
...
router.post('/delivered-order', function (req, res) {
  ;(async () => {
    try {
      let pool = await sql.connect(config)
      let result = await pool
        .request()
        .query(
          `Select * from HoaDon where NVGiaoHang = 
          '${req.session.user}' and TrangThai = N'Đã Giao'`,
        )
